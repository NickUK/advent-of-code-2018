...
while more:
  ...
  for o in fin[::-1]:
    options = options + list(filter(lambda x: x not in fin and x not in in_progress, ({k: v for k, v in rules.items() if o in v and o not in in_progress})))

  for option in sorted(options):
    if (option in rules and len(set(rules[option]) - set(fin)) == 0) or (option not in rules and option not in fin and option not in in_progress):
