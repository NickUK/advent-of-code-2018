...
def bfs(pos, avail, targets, max_xy):
  ...
  while len(visited) > 0:
    ...
    if arr[current[0][1]][current[0][0]] == HIGH_VAL:
      options = filter(lambda x: x in avail and arr[x[1]][x[0]] == HIGH_VAL, adjacent(current[0]))
...
def do_print(combs, walls, round, max_xy): 
  ...
  for y in range(max_xy[1]+1):
    ...
    output += "\t" + str(list(sorted(filter(lambda c: c.pos[1] == y, combs), key = lambda x: (x.pos[0])))).replace("[", "").replace("]", "")
  ...
  try:
    while True:
      ...
      for unit in ordered_units:
        ...
        targets = set(filter(lambda x: x.type != unit.type and x.hp > 0, ordered_units))
        ...
        if len(in_range) == 0:
          ...
          for y in range(1, max_xy[1]):
            for x in range(1, max_xy[0]):
              alive_units = filter(lambda x: x.hp > 0, ordered_units)
              if ((x, y) not in walls) and x is not 0 and x is not max_xy[0] and y is not 0 and y is not max_xy[1] and (x, y) not in map(lambda d: d.pos, alive_units):
                unnoc.add((x, y))
          unnoc.add((unit.pos))

          possible_target_squares = []
          for targ in targets:
            possible_target_squares.extend(filter(lambda x: x in unnoc, adjacent(targ.pos)))

          surrounding = list(map(lambda s: bfs(s, unnoc, possible_target_squares, max_xy), filter(lambda x: x in unnoc, adjacent(unit.pos))))
          surrounding = list(filter(lambda x: x[1] is not None, surrounding))
